...
task("task:deployOracle")
  ...
  .setAction(async function (taskArguments: TaskArguments, { ethers }) {
    ...
    const envConfig = dotenv.parse(fs.readFileSync("node_modules/fhevm/oracle/.env.oracle"));
...
task("task:computePredeployAddress")
  ...
  .setAction(async function (taskArguments: TaskArguments, { ethers }) {
    ...
    const envFilePath = path.join(__dirname, "../node_modules/fhevm/oracle/.env.oracle");
    const content = `ORACLE_CONTRACT_PREDEPLOY_ADDRESS=${oraclePredeployAddressPrecomputed}\n`;
    try {
      fs.writeFileSync(envFilePath, content, { flag: "w" });
      console.log("oraclePredeployAddress written to node_modules/fhevm/oracle/.env.oracle successfully!");
    } catch (err) {
      console.error("Failed to write to node_modules/fhevm/oracle/.env.oracle:", err);
    }
