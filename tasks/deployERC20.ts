...
task("task:deployERC20").setAction(async function (taskArguments: TaskArguments, { ethers }) {
  ...
  const erc20Factory = await ethers.getContractFactory("ConfidentialERC20");
  const encryptedERC20 = await erc20Factory.connect(signers[0]).deploy("Naraggara", "NARA"); // City of Zama's battle);
